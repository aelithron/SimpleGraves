...
public class CoreTools {
    ...
    public void setPlugin(SimpleGraves plugin) {
        this.plugin = plugin;
    }
    ...
    public void checkForUpdates() {
        if (plugin.getConfig().getBoolean("CheckForUpdates")) {
            HttpRequest check = HttpRequest.newBuilder()
                    .GET()
                    .uri(URI.create("https://api.github.com/repos/aelithron/SimpleGraves/releases/latest"))
                    .build();
            ...
            if (!version.equals(latestVersion)) {
                plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, () -> {
                    ...
                    plugin.getLogger().info("Please update at https://github.com/aelithron/SimpleGraves/releases!");
                }, 60L);
            }
        }
    }
